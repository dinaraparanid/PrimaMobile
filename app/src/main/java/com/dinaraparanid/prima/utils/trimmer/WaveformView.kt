...
internal class WaveformView(context: Context, attrs: AttributeSet) :
    ...
    private var lenByZoomLevel: IntArray? = null
    private var valuesByZoomLevel: Array<DoubleArray?>? = null
    ...
    internal fun setSoundFile(soundFile: SoundFile) {
        ...
        sampleRate = this.soundFile!!.sampleRate
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (heightsAtThisZoomLevel == null) computeIntsForThisZoomLevel()
        ...
        repeat(width) {
            ...
            drawWaveformLine(
                canvas, it,
                ctr - heightsAtThisZoomLevel!![start + it],
                ctr + 1 + heightsAtThisZoomLevel!![start + it],
                paint
            )
    ...
    private fun computeDoublesForAllZoomLevels() {
        val numFrames = soundFile!!.numFrames
        ...
        numZoomLevels = 5
        lenByZoomLevel = IntArray(5)
        zoomFactorByZoomLevel = DoubleArray(5)
        valuesByZoomLevel = arrayOfNulls(5)
        ...
        (1 until numFrames).forEach {
            valuesByZoomLevel!![0]!![it * 2] = 0.5 * (heights[it - 1] + heights[it])
            valuesByZoomLevel!![0]!![(it * 2) + 1] = heights[it]
        }

        // Level 1 is normal

        lenByZoomLevel!![1] = numFrames
        valuesByZoomLevel!![1] = DoubleArray(lenByZoomLevel!![1])
        zoomFactorByZoomLevel[1] = 1.0

        repeat(lenByZoomLevel!![1]) {
            valuesByZoomLevel!![1]!![it] = heights[it]
        }

        // 3 more levels are each halved

        (2..4).forEach { i ->
            lenByZoomLevel!![i] = lenByZoomLevel!![i - 1] / 2
            valuesByZoomLevel!![i] = DoubleArray(lenByZoomLevel!![i])
            zoomFactorByZoomLevel[i] = zoomFactorByZoomLevel[i - 1] / 2.0

            repeat(lenByZoomLevel!![i]) {
                valuesByZoomLevel!![i]!![it] =
                    0.5 * (valuesByZoomLevel!![i - 1]!![2 * it] +
                            valuesByZoomLevel!![i - 1]!![2 * it + 1])
            }
        }
    ...
    private fun computeIntsForThisZoomLevel() {
        ...
        heightsAtThisZoomLevel = IntArray(lenByZoomLevel!![zoomLevel])

        repeat(lenByZoomLevel!![zoomLevel]) {
            heightsAtThisZoomLevel!![it] =
                (valuesByZoomLevel!![zoomLevel]!![it] * halfHeight).toInt()
        }
    }
}